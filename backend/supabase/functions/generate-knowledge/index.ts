...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!userId) {
      return new Response(JSON.stringify({ error: "Unauthorized", requestId }), { status: 401, headers: corsHeadersWithJson });
    }

    if (!usageTrackingHandledByProxy(req)) {
      await consumeUsageOrThrow(supabaseAdmin, userId, 'knowledge_generate');
    }
