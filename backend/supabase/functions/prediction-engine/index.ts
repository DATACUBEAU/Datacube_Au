...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // NEW: Usage Guard
    if (userId) {
        await requireProEntitlementOrThrow(supabaseAdmin, userId, 'exam_predictions');
        if (!usageTrackingHandledByProxy(req)) {
          await consumeUsageOrThrow(supabaseAdmin, userId, 'predictions');
        }
    } else {
        return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: corsHeadersWithJson });
    }
    ...
    const aiResponse = await callAU(supabaseAdmin, systemPrompt, userPrompt, 0.4, false, modelOverride, {
