...
VALUES
  ('model_routing.tier_split_enabled', FALSE, 'billing', 'If true, free users route to free models and pro users route to paid models.', 'global', '{}'::jsonb),
  ('model_routing.paid_default_enabled', TRUE, 'billing', 'Safety default for paid routing when tier split is disabled.', 'global', '{}'::jsonb)
ON CONFLICT (key)
DO UPDATE
SET
...
-- 2) Reset unhealthy paid key and keep free key optionally available for split mode.
DO $$
BEGIN
  IF to_regclass('public.ai_provider_keys') IS NOT NULL THEN
    ...
    -- Optional hard-disable of free key in paid-for-all mode:
