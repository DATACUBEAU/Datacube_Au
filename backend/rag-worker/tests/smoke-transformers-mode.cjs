...
async function main() {
  ...
  const { data: docAfter, error: docAfterError } = await supabase
    .from('au_documents')
    .select('id,status,error,storage_deleted_at,source_deleted_at,source_cleanup_result,cleanup_pending,cleanup_last_error')
    .eq('id', documentId)
    .maybeSingle();
  if (docAfterError) {
  ...
  if (!docAfter.storage_deleted_at || !docAfter.source_deleted_at) {
    fail('Smoke assertion failed: source deletion timestamps were not recorded after completed ingestion.', {
      documentId,
      storage_deleted_at: docAfter?.storage_deleted_at || null,
      source_deleted_at: docAfter?.source_deleted_at || null,
      source_cleanup_result: docAfter?.source_cleanup_result || null,
    });
  }
  if (!['deleted', 'missing', 'already_deleted', 'no_source'].includes(String(docAfter.source_cleanup_result || ''))) {
    fail('Smoke assertion failed: source cleanup result is not a successful terminal value.', {
      documentId,
      source_cleanup_result: docAfter?.source_cleanup_result || null,
      cleanup_pending: docAfter?.cleanup_pending ?? null,
      cleanup_last_error: docAfter?.cleanup_last_error || null,
    });
  }
  if (docAfter.cleanup_pending === true) {
    fail('Smoke assertion failed: completed ingestion should not leave cleanup_pending=true after successful source deletion.', {
      documentId,
      source_cleanup_result: docAfter?.source_cleanup_result || null,
      cleanup_last_error: docAfter?.cleanup_last_error || null,
    });
