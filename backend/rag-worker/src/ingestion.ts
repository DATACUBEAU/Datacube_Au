...
export class IngestionService {
  ...
  private isMissingRelationError(error: any, relation: string): boolean {
    ...
    return (
      code === 'PGRST205' ||
      (message.includes('could not find the table') && message.includes(target)) ||
      (message.includes('relation') && message.includes(target) && message.includes('does not exist'))
    );
  }
  ...
  private async countQdrantPoints(documentId: string, ownerId: string): Promise<number> {
    const collectionName = 'au_chunks';
    try {
      const countRes = await this.withQdrantRetry('Qdrant reuse verification count', () =>
        this.qdrant.count(collectionName, {
          filter: this.ownerFilter(documentId, ownerId),
          exact: true,
        } as any)
      );
      return Number((countRes as any)?.count || 0);
    } catch (error: any) {
  ...
  private async findDocumentVersion(documentId: string, contentHash: string): Promise<string | null> {
    ...
    if (error) {
      if (this.isMissingRelationError(error, 'au_document_versions')) {
        return null;
      }
      throw error;
    }
  ...
  private async markDocumentVersionActive(documentId: string, versionId: string): Promise<void> {
    ...
    if (deactivate.error && !this.isMissingRelationError(deactivate.error, 'au_document_versions')) {
    ...
    if (activate.error && !this.isMissingRelationError(activate.error, 'au_document_versions')) {
      throw activate.error;
    }
  }

  private async upsertActiveDocumentVersion(documentId: string, contentHash: string): Promise<string | null> {
    const { data, error } = await this.supabase
      .from('au_document_versions')
      .upsert(
        {
    ...
    const versionId = typeof (data as any)?.id === 'string' ? (data as any).id : null;
    if (versionId) {
      await this.markDocumentVersionActive(documentId, versionId);
    }
    return versionId;
  }

  private async updateDocumentCompletion(documentId: string, expiresAt: number, contentHash: string): Promise<void> {
    const variants = [
      {
        status: 'completed',
        error: null,
    ...
    for (const payload of variants) {
      ...
      if (this.isMissingColumnError(result.error, 'content_hash')) {
    ...
    if (lastError) {
      logger.warn('Document completion update skipped content_hash column fallback', {
        documentId,
  ...
  private async tryReuseExistingEmbeddings(params: {
    documentId: string;
    ownerId: string;
    contentHash: string;
    ...
    const versionId = await this.findDocumentVersion(params.documentId, params.contentHash);
    if (!versionId) {
      return false;
    }

    const [existingChunkCount, existingVectorCount] = await Promise.all([
      this.countChunkRows(params.documentId, params.ownerId),
      this.countQdrantPoints(params.documentId, params.ownerId),
    ]);

    if (existingChunkCount !== params.expectedChunkCount || existingVectorCount !== params.expectedChunkCount) {
      return false;
    }

    await this.markDocumentVersionActive(params.documentId, versionId);
    await this.updateDocumentCompletion(params.documentId, params.expiresAt, params.contentHash);
    logger.info('Skipping embeddings for unchanged document version', {
      documentId: params.documentId,
      ownerId: params.ownerId,
      versionId,
      chunkCount: params.expectedChunkCount,
    });
    return true;
  }
  ...
  async processDocument(
    ...
    if (await this.tryReuseExistingEmbeddings({
      documentId,
      ownerId,
      contentHash,
      expectedChunkCount: chunkData.length,
      expiresAt,
    })) {
      return;
    }
    ...
    await this.upsertActiveDocumentVersion(documentId, contentHash);
    await this.updateDocumentCompletion(documentId, expiresAt, contentHash);
