...
exports.classifyAuthFailure = classifyAuthFailure;
exports.isAuthenticationFailure = isAuthenticationFailure;
exports.isAuthorizationFailure = isAuthorizationFailure;
const api_contract_1 = require("../api/api-contract");
...
function pushFragment(out, value) {
...
function collectTextFragments(value, out = [], depth = 0) {
    if (out.length >= 24 || depth > 3 || value == null)
        return out;
    if (typeof value === 'string') {
    ...
    if (value instanceof Error) {
        ...
        collectTextFragments(value.code, out, depth + 1);
        collectTextFragments(value.details, out, depth + 1);
        collectTextFragments(value.reason, out, depth + 1);
        return out;
    }
...
function inferReason(status, text, explicitReason) {
    if (explicitReason) {
        return explicitReason.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || (status === 401 ? 'unauthorized' : 'forbidden');
    }
    if (status === 401) {
        if (/missing token/i.test(text))
            return 'missing_token';
        if (/invalid token/i.test(text))
            return 'invalid_token';
        if (/session expired|re-?authentication required|re-?login required/i.test(text))
...
function classifyAuthFailure(input) {
    const parsed = (0, api_contract_1.extractApiError)(input, '');
    const detailsRecord = isRecord(parsed.details) ? parsed.details : null;
    const rootRecord = isRecord(input) ? input : null;
    const originalCodeRaw = typeof parsed.code === 'string' && parsed.code.trim()
        ? parsed.code
        : typeof rootRecord?.code === 'string'
            ...
            : typeof rootRecord?.error === 'string'
                ? rootRecord.error
                : '';
    const originalCode = originalCodeRaw ? (0, api_contract_1.normalizeApiErrorCode)(originalCodeRaw, '') : null;
    ...
    const isAuthenticationFailure = statusHint === 401 ||
        hasAuthenticationCode ||
        (hasAuthenticationText && !hasAuthorizationCode);
    const isAuthorizationFailure = statusHint === 403 ||
        hasAuthorizationCode ||
        (!isAuthenticationFailure && hasAuthorizationText);
    if (!isAuthenticationFailure && !isAuthorizationFailure) {
        return null;
    }
    const status = isAuthenticationFailure ? 401 : 403;
    const explicitReason = typeof detailsRecord?.reason === 'string'
...
function isAuthorizationFailure(input) {
