...
exports.serializeEffectivePlanLimitRuleMap = serializeEffectivePlanLimitRuleMap;
...
async function loadPlanLimitCatalog(supabase) {
    ...
    const legacyResults = await Promise.all(plan_limit_model_1.DEFAULT_PLAN_ORDER.map((plan) => loadLegacyPlanRuleSet(supabase, plan)));
    for (let i = 0; i < plan_limit_model_1.DEFAULT_PLAN_ORDER.length; i++) {
        const plan = plan_limit_model_1.DEFAULT_PLAN_ORDER[i];
...
function resolveEffectivePlanFromInputs(input) {
    ...
    const isMirroredExpired = typeof mirroredExpiresAt === 'string' &&
        new Date(mirroredExpiresAt).getTime() < nowMs;
    const isEntitlementExpired = typeof entitlementEndsAt === 'string' &&
...
function serializeEffectivePlanLimitRuleMap(limitRules) {
    return plan_limit_model_1.APPROVED_LIMIT_KEYS.reduce((acc, key) => {
