...
exports.ADMIN_ASSIGNABLE_PLAN_KEYS = exports.USER_ROLES = exports.ACCOUNT_STATUSES = void 0;
...
exports.normalizeAdminAssignablePlan = normalizeAdminAssignablePlan;
exports.adminAssignablePlanLabel = adminAssignablePlanLabel;
exports.resolveAdminPlanChangeType = resolveAdminPlanChangeType;
...
exports.ADMIN_ASSIGNABLE_PLAN_KEYS = ['free', 'pro_monthly', 'premium'];
const ADMIN_ASSIGNABLE_PLAN_SET = new Set(exports.ADMIN_ASSIGNABLE_PLAN_KEYS);
...
function normalizeAdminAssignablePlan(value) {
    const normalized = String(value ?? '').trim().toLowerCase();
    if (normalized === 'free')
        return 'free';
    if (normalized === 'premium')
        return 'premium';
    if (normalized === 'pro' || normalized === 'monthly' || normalized === 'pro_monthly')
        return 'pro_monthly';
    return ADMIN_ASSIGNABLE_PLAN_SET.has(normalized) ? normalized : null;
}
function adminAssignablePlanLabel(value) {
    if (value === 'premium')
        return 'Premium';
...
function resolveAdminPlanChangeType(input) {
