...
async function main() {
    ...
    await run('sessions inside the proactive refresh window refresh before the token actually expires', () => {
        ...
        const expiringSoon = {
            ...
            refresh_token: 'refresh',
            expires_in: 60,
            expires_at: Math.floor((now + browser_session_js_1.SUPABASE_SESSION_REFRESH_WINDOW_MS - 1000) / 1000),
            user: { id: 'user-1' },
        };
        strict_1.default.equal((0, browser_session_js_1.shouldRefreshSupabaseSession)(expiringSoon, now, browser_session_js_1.SUPABASE_SESSION_REFRESH_WINDOW_MS), true);
    });
    await run('healthy sessions outside the proactive refresh window do not refresh eagerly', () => {
        ...
        const healthy = {
            ...
            refresh_token: 'refresh',
            expires_in: 60,
            expires_at: Math.floor((now + browser_session_js_1.SUPABASE_SESSION_REFRESH_WINDOW_MS + 120000) / 1000),
            user: { id: 'user-1' },
        };
        strict_1.default.equal((0, browser_session_js_1.shouldRefreshSupabaseSession)(healthy, now, browser_session_js_1.SUPABASE_SESSION_REFRESH_WINDOW_MS), false);
    });
    if (failed > 0) {
        process.exit(1);
    }
