...
exports.GenerationHandler = void 0;
...
class GenerationHandler {
    ...
    async handleKnowledge(body, headers, reply) {
        ...
        try {
            ...
            utils_js_1.logger.error('knowledge generation error', err.message);
    ...
    async handleExamPredictions(body, headers, reply) {
        ...
        try {
            ...
            utils_js_1.logger.error('exam predictions error', err.message);
            return reply.code(500).send({ error: 'generation_failed', message: err.message });
        }
    }
    async handlePracticeExam(body, headers, reply) {
        ...
        try {
            ...
            utils_js_1.logger.error('practice exam error', err.message);
            return reply.code(500).send({ error: 'generation_failed', message: err.message });
        }
    }
    async handlePromptStarters(body, headers, reply) {
        const userId = headers['x-user-id'];
        ...
        try {
            ...
            utils_js_1.logger.error('prompt starters error', err.message);
            return reply.code(500).send({ error: 'generation_failed', message: err.message });
        }
    }
    ...
    buildKnowledgePrompt(documentContent, pastQuestionsContent) {
        ...
        prompt += `\n\nGenerate a comprehensive knowledge summary in JSON format with the following structure:
{
  ...
  "practice_questions": [...]
    ...
    buildPredictionsPrompt(textbookContent, pastQuestionsContent) {
        ...
        prompt += `\n\nGenerate exam predictions in JSON format:
{
  ...
  "reasoning": "..."
    ...
    buildPracticeExamPrompt(documentContent, pastQuestionsContent) {
        ...
        prompt += `\n\nGenerate a practice exam in JSON format:
{
  ...
  "questions": [
    ...
    buildPromptStartersPrompt(title, content, userIdea) {
        if (userIdea) {
    ...
    async generate(candidate, prompt) {
    ...
    async callOpenRouter(candidate, messages) {
    ...
    async callAnthropic(candidate, messages) {
...
exports.GenerationHandler = GenerationHandler;
