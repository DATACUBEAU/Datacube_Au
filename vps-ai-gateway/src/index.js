...
const VPS_SHARED_SECRET = (0, auth_js_1.resolveVpsSharedSecret)();
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    utils_js_1.logger.error('Missing SUPABASE_URL or SUPABASE_ANON_KEY');
    process.exit(1);
}
if (!VPS_SHARED_SECRET.ok) {
    utils_js_1.logger.error('AI gateway authentication is not configured', { code: VPS_SHARED_SECRET.code });
    process.exit(1);
}
...
async function buildServer() {
    ...
    server.addHook('preHandler', async (request, reply) => {
        ...
        const expectedRoute = (0, auth_js_1.routeRequirementForPath)(path);
        if (!expectedRoute) {
        ...
        const ticketData = await (0, auth_js_1.verifyVpsTicket)(token, VPS_SHARED_SECRET_VALUE, expectedRoute);
        if (!ticketData) {
