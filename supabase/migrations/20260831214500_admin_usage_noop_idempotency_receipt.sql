-- Persist successful zero-delta target adjustments as immutable ledger receipts.
--
-- A set/reset that already matches current usage is still a completed logical
-- mutation. If its response is lost, the same request id must resolve to that
-- original completion rather than being recalculated against newer usage.
-- Reuse the authoritative append-only adjustment ledger rather than introducing
-- a parallel idempotency store: a zero delta changes no usage total, while the
-- existing unique request key and replay fingerprint make the completion durable.
