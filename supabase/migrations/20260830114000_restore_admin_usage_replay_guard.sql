-- Restore idempotency-fingerprint validation in the final composed admin-usage wrappers.
-- The checkpoint/reservation composition migration replaced the versioned wrappers and
-- accidentally omitted admin_assert_usage_adjustment_replay. Keep replay validation,
-- in-flight reservation protection, legacy checkpointing, and checked adjustment writes
-- under the same per-user mutation-version serialization boundary.
...
BEGIN
  ...
  -- The replay fingerprint must be checked while the same usage-version row is held,
  -- before checkpointing or attempting an insert/dedupe in the checked implementation.
...
BEGIN
  ...
  -- Validate every retry fingerprint before any checkpoint/event write so a batch with
  -- one mismatched request ID fails atomically without partial accounting side effects.
