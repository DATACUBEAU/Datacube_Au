-- Persist zero-effective decrease operations as durable idempotency receipts.
--
-- A decrease against already-zero usage is a completed logical request. If its
-- response is lost and later usage accrues, replaying the same request id must
-- recover that completion rather than decreasing the newer usage. Keep the
-- receipt in the authoritative append-only adjustment ledger and preserve the
-- action as `decrease` so audit semantics remain truthful.
...
ALTER TABLE public.au_usage_admin_adjustments
  ...
  CHECK (
    ...
    OR (
      delta = 0
      AND action IN ('decrease', 'set', 'reset')
...
BEGIN
  ...
  -- Increase remains strictly positive. Decrease may be zero only so an
  -- already-zero relative decrease can be recorded as a durable completion.
