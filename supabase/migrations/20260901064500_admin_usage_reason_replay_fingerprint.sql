-- Bind idempotent admin-usage replays to the canonical audit reason as well as
-- the action/window/amount fingerprint. The immutable request identity must not
-- report success for a retry whose justification differs from the persisted audit.
  ...
  IF FOUND THEN
    IF v_existing.reason IS DISTINCT FROM v_reason THEN
      RAISE EXCEPTION 'usage_adjustment_idempotency_conflict'
        USING ERRCODE = '22023',
              DETAIL = 'The request ID was already used with a different audit reason.';
    END IF;
