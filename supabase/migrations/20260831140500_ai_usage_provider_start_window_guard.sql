-- Require every provider start, including the first accepted attempt, to remain inside
-- the finite quota windows under which the reservation was originally admitted.
-- Exact reservation replay can recover a lost reservation response across a reset boundary,
-- but provider execution itself must not begin against an expired daily/hourly/weekly/monthly/custom window.
...
BEGIN
  ...
  -- Provider-start decisions use the actual time after daily -> lifetime -> reservation
  -- serialization, not the transaction-start timestamp.
  ...
  -- Reservation replay remains recoverable across reset boundaries, but provider execution
  -- itself may start only while every finite authoritative admission window is still active.
  -- This applies equally to the first provider attempt and to later takeovers. Lifetime checks
  -- have no finite end bound, and rolling-deploy rows without window metadata keep compatibility.
