-- Prevent an older superseded provider attempt from committing a newer takeover's reservation.
-- Commit keeps the established daily counter -> lifetime total -> reservation lock order,
-- then validates the caller ticket against the ticket currently bound to the reservation
-- before either the active transition or committed replay path can succeed.
