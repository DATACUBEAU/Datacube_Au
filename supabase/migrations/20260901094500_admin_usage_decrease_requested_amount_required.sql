-- Require immutable caller intent for every new decrease adjustment. Historical
-- completed rows remain replayable through the compatibility branches below, but
-- new zero/clamped decreases cannot create an amount-less receipt whose request ID
-- could later be reused with different relative intent.
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    IF v_action = 'decrease' AND v_requested_amount IS NULL THEN
      RAISE EXCEPTION 'usage_adjustment_requested_amount_required'
        USING ERRCODE = '22023',
              DETAIL = 'Decrease operations require an immutable requested_amount fingerprint.';
    END IF;
  ...
  IF v_existing.action <> v_action
     ...
     OR (
       ...
       AND NOT (
         ...
         OR
         -- Compatibility for completed rows written before requested_amount was
         -- mandatory. Existing no-op receipts remain terminal/no-effect; older
         -- non-no-op rows retain their original signed-delta replay identity.
