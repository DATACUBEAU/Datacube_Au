-- Validate provider settlement against wall-clock time after accounting serialization.
-- PostgreSQL now() is transaction-stable, so a commit that begins before lease expiry but
-- waits on accounting/reservation locks until afterward must not be allowed to settle late.
...
BEGIN
  ...
  -- Completed work is durably represented by its usage event and remains replayable even
  -- after the lease. Only a still-provisional reserved -> committed transition requires
  ...
  IF v_row.status = 'reserved' THEN
    ...
    -- Capture real execution time only after daily -> lifetime -> reservation serialization.
    -- A transaction that started before expiry but waited until afterward must fail closed.
    v_settlement_now := clock_timestamp();

    IF v_effective_expiry <= v_settlement_now THEN
      RETURN jsonb_build_object(
        'ok', FALSE,
  ...
  SET status = 'committed',
      ...
      committed_at = v_settlement_now,
      updated_at = v_settlement_now
  WHERE id = v_row.id
