-- Serialize admin usage corrections and make set/reset semantics optimistic-concurrency safe.
--
-- The API calculates a target from canonical effective usage. Without a database
-- compare-and-write boundary, two distinct concurrent set/reset requests can both
-- derive deltas from the same stale value and overshoot. These RPCs serialize by
-- user/metric/window, verify the adjustment total observed by the caller, preserve
-- request-id idempotency, and make reset-all transactional.
...
BEGIN
  ...
  IF p_delta = 0 AND v_action NOT IN ('set', 'reset') THEN
    RAISE EXCEPTION 'zero_delta_only_allowed_for_target_actions' USING ERRCODE = '22023';
  END IF;
  ...
  -- Transaction-scoped lock: distinct request IDs for the same quota window cannot
  -- compare and append simultaneously. Other users/metrics/windows remain independent.
  ...
  -- Idempotent retries win before the optimistic concurrency check. A retry of an
  -- already-committed request must remain successful even though the total changed.
  ...
  -- A set/reset that is already at its requested target still participates in the
  -- compare-and-write boundary but does not invent a zero-value audit event.
  IF p_delta = 0 THEN
    RETURN jsonb_build_object(
      'ok', TRUE,
      'deduped', FALSE,
...
GRANT EXECUTE ON FUNCTION public.admin_adjust_usage_checked(UUID, TEXT, UUID, TEXT, NUMERIC, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, NUMERIC, JSONB) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.admin_adjust_usage_batch_checked(UUID, TEXT, UUID, TEXT, JSONB) TO authenticated;
