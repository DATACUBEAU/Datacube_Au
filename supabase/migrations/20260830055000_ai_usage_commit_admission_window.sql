-- Keep committed AI usage in the quota window where it was admitted.
--
-- Reservations are quota-admitted at reservation creation time. Recording the
-- durable usage event with commit-time `now()` can move a request across an
-- hourly/daily/weekly/monthly/custom reset boundary after it has already been
-- admitted under the previous window. Persist the event at the immutable
-- reservation creation timestamp instead, while retaining commit time on the
-- reservation row for operational latency/audit purposes.
