-- Evaluate provider-attempt lease expiry and takeover cooldown against serialized wall-clock time.
--
-- PostgreSQL now() is transaction-stable. begin_ai_usage_reservation can wait on the
-- daily counter, lifetime total, and reservation row locks; using transaction-start time
-- after that wait can accidentally extend an expired reservation or a retry cooldown.
-- Capture one wall-clock instant only after the existing lock order is fully held and use
-- it consistently for expiry, cooldown, attempt timestamps, and audit timestamps.
...
BEGIN
  ...
  -- Capture freshness only after daily -> lifetime -> reservation serialization.
