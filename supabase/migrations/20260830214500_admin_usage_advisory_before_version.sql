-- Prevent admin usage corrections from deadlocking live AI reservation accounting.
--
-- AI admission holds usage counter/lifetime rows, then the exact quota-window
-- advisory lock, and counter mutation triggers acquire the per-user mutation-version
-- row. The exposed admin wrappers previously acquired that version row first and
-- only reached the same advisory lock inside admin_adjust_usage_*_checked. That
-- creates a cycle even when no legacy checkpoint write is needed:
--   AI:    counter -> total -> advisory -> version
--   admin: version -> advisory
--
-- Acquire every quota-window advisory lock before the mutation-version row so the
-- admin path cannot hold version while waiting on an AI-held advisory lock. Batch
-- locks are acquired in deterministic bigint order to avoid cross-window admin
-- deadlocks. Lower-level checked RPCs keep their existing advisory calls; PostgreSQL
-- transaction advisory locks are safely re-entrant for the owning transaction.
...
BEGIN
  ...
  -- Match the exact lock identity used by admin_adjust_usage_checked and
  -- canonical AI quota admission, but acquire it before the version row.
...
BEGIN
  ...
  -- Take all exact quota-window locks before the shared mutation-version row.
  -- Sorting the bigint lock identities gives every batch the same acquisition
  -- order regardless of UI item order.
  FOR v_lock_key IN
  ...
  -- Validate every idempotency fingerprint before any checkpoint write.
