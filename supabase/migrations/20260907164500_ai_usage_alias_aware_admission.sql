-- Keep atomic AI quota admission aligned with canonical usage alias resolution.
--
-- Historical tracked usage may exist under accepted aliases (for example used_chats
-- or used_tokens) while the canonical metric bucket is absent. Canonical usage reads
-- those aliases in priority order, but the reservation boundary previously queried
-- only the canonical key for daily, lifetime, and event-window usage. That could make
-- the atomic gate observe less usage than the dashboard/entitlement resolver and admit
-- work beyond the effective cap. Reuse the existing database alias helper and the same
-- canonical-first JSON resolver used by usage checkpointing; do not sum aliases because
-- modern events/counters intentionally write canonical and compatibility aliases together.

BEGIN;

CREATE OR REPLACE FUNCTION public.reserve_ai_usage_window_unchecked(
  p_user_id UUID,
  p_feature_key TEXT,
  p_route TEXT,
...
BEGIN
  ...
  LOOP
    ...
    v_metric_aliases := public.admin_usage_metric_aliases(v_metric_key);
    ...
    IF v_counter_scope = 'today' THEN
      v_current := public.admin_usage_json_metric_value(COALESCE(v_today, '{}'::jsonb), v_metric_aliases);
    ELSIF v_counter_scope = 'window' AND v_window_start IS NOT NULL THEN
      ...
      v_current := public.admin_usage_json_metric_value(COALESCE(v_window_totals, '{}'::jsonb), v_metric_aliases);

      -- Active reservations are modern authoritative rows and contain the canonical
      -- increment. Read only that canonical bucket here so compatibility aliases are
    ...
    ELSE
      v_current := public.admin_usage_json_metric_value(COALESCE(v_total, '{}'::jsonb), v_metric_aliases);
    END IF;
